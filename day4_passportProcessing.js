...
// turn each passport into an object
function formatPassports() {
  ...
  for(let i = 0; i < passports.length; i++) {
    ...
    strings = passports[i].split(/\s+/);

    strings.forEach(field => {
      let f = field.substring(0,3);
      let content = field.substring(4)
      passportObject[f] = content;
    })
  
    formattedPassports.push(passportObject);
  }

  return formattedPassports;
}

const allPassports = formatPassports();

// filter out only passports with all of the correct fields
function processPassports() {
  const numReqFields = requiredFields.length;
  let validPassports = [];

  for(let i = 0; i < allPassports.length; i++) {
    let passportFields = Object.keys(allPassports[i]);
    let filteredFields = requiredFields.filter(value => passportFields.includes(value))

    if(filteredFields.length == numReqFields) {
      validPassports.push(allPassports[i]);
    }
  }
...
console.log('Passports with all fields: ', processPassports().length);

let processedPassports = processPassports();

function checkHeight(height) {
  if(height.indexOf('cm') >= 0) {
    let cm = parseInt(height.substring(0, height.length - 1))
    if (cm >= 150 && cm <= 193) {
      return true;
    }
  }
  if(height.indexOf('in') >= 0) {
    let inches = parseInt(height.substring(0, height.length - 1))
...
console.log('Passports meet requirements: ', validPassports().length);